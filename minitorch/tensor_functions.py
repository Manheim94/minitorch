...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        x = t1.f.sigmoid_map(t1)
        ones = grad_output.zeros(tuple(x.shape))
        ones._tensor._storage[:] = 1
        return x*(ones-x)*grad_output
        # or return x*(1-x)*grad_output
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        order = order._tensor._storage.astype(int)
        ctx.save_for_backward(order)
        return a._new(a._tensor.permute( *order.tolist() ))

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (order,) = ctx.saved_values
        reverse = np.argsort( order )
        return (grad_output.permute( reverse.tolist() ), 0.0)
